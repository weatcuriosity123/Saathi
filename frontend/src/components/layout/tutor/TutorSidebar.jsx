...
const navItems = [
  ...
  { icon: "school", label: "My Courses", href: "#" }, // Placeholder
  { icon: "cloud_upload", label: "Upload Course", href: "/create-course" },
  { icon: "analytics", label: "Analytics", href: "#" }, // Placeholder
  { icon: "payments", label: "Earnings", href: "#" }, // Placeholder
      ...
      <div className="pt-6 mt-6 border-t border-outline-variant/20 space-y-2">
        {secondaryItems.map((item) => (
          <Link
            key={item.label}
            href={item.href}
            className="flex items-center gap-3 px-4 py-2 text-on-surface-variant hover:bg-white/50 rounded-lg transition-colors group"
          >
