...
/** Fallback avatar when user has no Cloudinary avatar set */
...
export default function Navbar() {
  ...
  const avatarSrc = user?.avatar || dicebearAvatar(user?.name || "guest");
  ...
  // While session is restoring, render nothing in the auth section
  // to prevent a flash of the wrong state (logged-out buttons on a logged-in user)
  const authSection = loading ? (
    ...
    <>
      <Link
        href="/login"
        className="text-primary font-bold px-4 py-2 hover:bg-primary/5 rounded-xl transition-all active:scale-95"
      >
        Sign In
      </Link>
      <Link href="/signup">
      ...
      {isProfileOpen && (
        <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-ambient border border-outline-variant/20 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="p-5 border-b border-outline-variant/10 bg-surface-container-low/30 text-center">
            ...
            {user?.role !== "student" && (
              <span className="inline-block mt-1 text-[10px] font-black uppercase tracking-widest text-primary bg-primary/10 px-2 py-0.5 rounded-full">
                {user?.role}
              </span>
            )}
          </div>
          <div className="p-2">
            <Link
              ...
              className="flex items-center gap-3 px-4 py-3 text-sm font-semibold text-on-surface-variant hover:text-primary hover:bg-primary/5 rounded-xl transition-all"
            >
              <span className="material-symbols-outlined text-[20px] font-['Material_Symbols_Outlined']">
                dashboard
              </span>
              Dashboard
            </Link>
      ...
      <div className="flex justify-between items-center h-20 px-6 md:px-10 max-w-[1440px] mx-auto font-headline antialiased tracking-tight">
        ...
        <div className="flex items-center gap-4 lg:gap-10">
          ...
          <div className="hidden lg:flex items-center bg-surface border border-outline-variant px-4 py-2.5 rounded-xl w-80 group focus-within:ring-2 ring-primary/20 transition-all">
            <span className="material-symbols-outlined text-outline mr-2 text-[20px] font-['Material_Symbols_Outlined']">
              search
            </span>
            <input
            ...
            {searchQuery && (
              <button
                ...
                className="material-symbols-outlined text-outline hover:text-primary text-[18px] font-['Material_Symbols_Outlined']"
              >
                close
              </button>
            )}
              ...
              <Link
                ...
                className={`flex items-center justify-between px-4 py-4 rounded-2xl transition-all ${
                  isActive(link.href)
                    ...
                    : "text-on-surface-variant font-bold hover:bg-surface"
                }`}
              >
                {link.name}
                {isActive(link.href) && (
            ...
            {!loading && !isLoggedIn ? (
              <>
                <Link
                  onClick={() => setIsOpen(false)}
                  href="/login"
                  className="text-primary font-bold text-center py-4 bg-primary/5 rounded-2xl transition-all"
                >
                  Sign In
                </Link>
              ...
              !loading && (
                ...
                >
                  <span className="material-symbols-outlined font-['Material_Symbols_Outlined']">
                    logout
                  </span>
                  Sign Out
                </button>
